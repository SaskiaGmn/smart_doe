...
class GPOptimizer():
    ...
    def optimization_loop(self, num_restarts: int, raw_samples: int, convergence_criteria: str = 'obj_func_pred_opt', manual_input: bool = True, max_iterations: int = 100):
        for iteration in range(max_iterations):
            ...
            if manual_input:
                print(f"\nProposed parameters for iteration {iteration + 1}:")
                for i, value in enumerate(candidate.squeeze().tolist()):
                    print(f"Parameter {i+1}: {value:.4f}")
                
                while True:
                    try:
                        next_y_value = float(input("\nPlease enter the observed value: "))
                        break 
                    except ValueError:
